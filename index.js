...
console.log-ing the following pieces of data 
...
function getFinals(data) {
    const finals = data.filter((final) => {return final["Stage"] == "Final";});
    return finals;
}
...
accepts the callback function `getFinals()` and determine the winner 
(home or away) of each `finals` game. Return the name of all winning 
...
function getWinners(callBack, data) {
    ...
    let newArray = array.map((data) => {
        if (data["Home Team Goals"] > data["Away Team Goals"]){
            ...
            return data[`${["Home Team Name"]} tied with ${["Away Team Name"]}`];
        }
        else(data["Home Team Goals"] < data ["Away Teal Goals"])
...
function getWinnersByYear(cb1, cb2) {
    const winner = cb1(getFinals, fifaData);
    const year = cb2(getFinals, fifaData);

    for (let i = 0; i < winner.length; i++); {

  console.log(`In ${year[5]},${winner[5]} won the world cup!`);
} 
};
...
parameter `data` and returns the the average number of home team goals
 and away team goals scored per match (Hint: use .reduce and do this in 
    2 steps) */

    function getAverageGoals(data) {

        let homeAverage = data.reduce((total, goals) => {
            return total += goals['Home Team Goals'];
        }, 0)
    
        let awayAverage = fifaData.reduce((total, goals) => {
            return total += goals['Away Team Goals'];
        }, 0)
        console.log(homeAverage / data.length);
        console.log(awayAverage / data.length);
